...
class App extends Component {
  state = {songName: '', tracksList: initialTracksList}
  ...
  onRemoveItem = id => {
    ...
    const filteredUserData = tracksList.filter(eachItem => eachItem.id !== id)
    this.setState({
      tracksList: filteredUserData,
    })
  }

  render() {
    const {songName, tracksList} = this.state
    const filteredList = tracksList.filter(eachItem =>
      eachItem.name.toLowerCase().includes(songName.toLowerCase()),
    )
    return (
                  ...
                  <li className="list-item" key={eachItem.id}>
                    <div className="image-name-genre-container">
                      ...
                      <div>
                        ...
                        <p className="item-genre">{eachItem.genre}</p>
                      </div>
                    </div>
                    <div className="duration-delete-icon-container">
                      <p className="item-duration">{eachItem.duration}</p>
                      <button
                        data-testid="delete"
                        className="delete-button"
                        type="button"
                        onClick={() => this.onRemoveItem(eachItem.id)}
                      >
